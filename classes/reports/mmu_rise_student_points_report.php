...
class mmu_rise_student_points_report extends \block_reportsdash\report {
...
*/
        ...
        $sql = "SELECT 		
			                    SUM(pointvalue) as pointsvalue, up.userid, up.ruleid, up.rulelevelid, u.firstname, u.lastname, u.username,
			                    r.name AS 'rulename', 
                                t1.name AS 'level',
                                t2.name AS 'theme',
                ...
                FROM		mdl_local_mmu_rise  up 	JOIN
                            ...
                            mdl_local_mmu_rise_level_tags lt ON (rl.levelid = lt.id {$leveltagfiltersql}) JOIN
                            mdl_tag t1 ON (lt.tagid = t1.id) JOIN 
                            mdl_local_mmu_rise_rule r ON (up.ruleid = r.id)  JOIN
                            mdl_user u ON (up.userid = u.id {$firstnamefiltersql}  {$lastnamefiltersql} ) JOIN
                            mdl_local_mmu_rise_theme_tags theme ON (r.themeid = theme.id {$themetagfiltersql})  JOIN
                ...
                {$timecreatedsql}
                GROUP BY up.userid, up.ruleid, up.rulelevelid
                {$order_by} ";
    ...
    function get_filter_form() {
        ...
        $this->filterform = new \local_mmu_rise_reportsdash_reports\forms\mmu_rise_student_points_report_filter_form(null, array('rptname'=>"local_mmu_rise_reportsdash_reports\\reports\\mmu_rise_student_points_report"),
